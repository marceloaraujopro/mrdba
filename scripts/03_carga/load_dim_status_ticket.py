...
def ler_curated(caminho: Path) -> pd.DataFrame:
    ...
    if "status_ticket" not in df.columns:
        raise ValueError("Coluna 'status_ticket' não encontrada no curated")

    return df


def extrair_status(df: pd.DataFrame) -> pd.DataFrame:
    serie = (
        df["status_ticket"]
        .fillna("")
        .astype(str)
    ...
    return pd.DataFrame({"nm_status_ticket": serie})


def buscar_existentes(engine) -> set[str]:
    sql = text(f"SELECT nm_status_ticket FROM {SCHEMA}.{TABELA}")
...
def filtrar_novos(df: pd.DataFrame, existentes: set[str]) -> pd.DataFrame:
    return df[~df["nm_status_ticket"].isin(existentes)].copy()
...
def main() -> int:
    if len(sys.argv) != 3:
        print("Uso: python3 load_dim_status_ticket.py YYYY MM")
        return 1
    ...
    try:
        ...
        df_status = extrair_status(df)
        print(f"Status únicos encontrados: {len(df_status)}")

        engine = criar_engine()
        existentes = buscar_existentes(engine)

        df_novos = filtrar_novos(df_status, existentes)
        print(f"Novos status a inserir: {len(df_novos)}")
        ...
        print("-" * 70)
        print("Carga da dim_status_ticket concluída com sucesso.")
        return 0
