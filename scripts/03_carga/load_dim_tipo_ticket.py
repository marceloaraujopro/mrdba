...
def ler_curated(caminho: Path) -> pd.DataFrame:
    ...
    if "tipo_ticket" not in df.columns:
        raise ValueError("Coluna 'tipo_ticket' não encontrada no curated")

    return df


def extrair_tipos(df: pd.DataFrame) -> pd.DataFrame:
    serie = (
        df["tipo_ticket"]
        .fillna("")
        .astype(str)
    ...
    return pd.DataFrame({"nm_tipo_ticket": serie})


def buscar_existentes(engine) -> set[str]:
    sql = text(f"SELECT nm_tipo_ticket FROM {SCHEMA}.{TABELA}")
...
def filtrar_novos(df: pd.DataFrame, existentes: set[str]) -> pd.DataFrame:
    return df[~df["nm_tipo_ticket"].isin(existentes)].copy()
...
def main() -> int:
    if len(sys.argv) != 3:
        print("Uso: python3 load_dim_tipo_ticket.py YYYY MM")
        return 1
    ...
    try:
        ...
        df_tipos = extrair_tipos(df)
        print(f"Tipos únicos encontrados: {len(df_tipos)}")

        engine = criar_engine()
        existentes = buscar_existentes(engine)

        df_novos = filtrar_novos(df_tipos, existentes)
        print(f"Novos tipos a inserir: {len(df_novos)}")
        ...
        print("-" * 70)
        print("Carga da dim_tipo_ticket concluída com sucesso.")
        return 0
